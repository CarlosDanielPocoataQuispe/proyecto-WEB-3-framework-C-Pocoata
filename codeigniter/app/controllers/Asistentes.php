...
class Asistentes extends BaseController
{
    public function index()
    {
        ...
        $asistentes = $model->findAll();
        
        // Obtener nombres de eventos
        foreach ($asistentes as $asistente) {
            $evento = $eventoModel->find($asistente->evento_id);
            $asistente->evento_nombre = $evento ? $evento->nombre : 'No encontrado';
        }
        
        $data['registros'] = $asistentes;
        $data['titulo'] = 'Asistentes';
        $data['tipo'] = 'asistentes';
    ...
    private function cargarVista($vista, $data = [])
    {
        ...
        echo $this->generarLayout($data, function() use ($vista, $data) {
            return view('asistentes/' . $vista, $data);
        });
    }
